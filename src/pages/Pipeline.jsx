...
export default function Pipeline() {
  ...
  const forecast = useMemo(() => {
    ...
    // Commit falls back to the highest-probability open stage if no stage is
    // literally named "Negotiation" — still driven off real stage probabilities.
    ...
    <AppShell>
      <div className="ikyam-mock pipeline-page">
        <div className="scr-head">
          ...
          <span className="goal">Drag a card to a new stage, or double-click it to open its Record page.</span>
        </div>

        <div className="rowx sp" style={{ flexWrap: 'wrap', gap: 8 }}>
          <div className="tiny">{totalDeals} open deal{totalDeals === 1 ? '' : 's'} · {columns.length} stages</div>
          <div className="rowx pipeline-viewtoggle">
        ...
        <div className="card pipeline-strip">
          <div><span className="lab">Commit</span><div className="mono pipeline-strip-num">₹{Math.round(forecast.commit).toLocaleString('en-IN')}</div></div>
          <div><span className="lab">Best case</span><div className="mono pipeline-strip-num">₹{Math.round(forecast.bestCase).toLocaleString('en-IN')}</div></div>
          <div><span className="lab">Open pipeline</span><div className="mono pipeline-strip-num">₹{Math.round(forecast.openPipeline).toLocaleString('en-IN')}</div></div>
          <div className="pipeline-strip-ai">
        ...
        {view === 'board' && (
          <div className="kwrap frame" style={{ marginTop: 13 }}>
            <div className="kcols" style={{ gridTemplateColumns: `repeat(${columns.length || 1},1fr)` }}>
              {columns.map((col) => (
                ...
                >
                      ...
                      >
                        ...
                        <div className="rowx sp" style={{ marginTop: 6 }}>
                          ...
                          <div className="rowx" style={{ gap: 6 }}>
                            ...
                            <span className={`av ${av.cls}`}>{av.initials}</span>
                          </div>
                        </div>
                        <select
                          value={card.stage_id}
                  ...
                  {col.cards.length === 0 && <div className="tiny" style={{ padding: '4px 2px' }}>No deals in this stage.</div>}
                </div>
              ))}
              {columns.length === 0 && <div className="tiny">Loading pipeline…</div>}
            </div>
          </div>
        )}

        {view === 'table' && (
          <div className="frame" style={{ marginTop: 13, padding: 4 }}>
            <table className="qtable">
              <thead><tr><th>Deal</th><th>Account</th><th>Stage</th><th>Win %</th><th className="num">Amount ₹</th></tr></thead>
              <tbody>
                {columns.flatMap((col) => col.cards.map((card) => (
                  <tr key={card.id} style={{ cursor: 'pointer' }} onClick={() => navigate(`/record/${card.id}`)}>
                    ...
                    <td><span className="chip">{col.stage.name}</span></td>
                    <td>{(card.win_probability ?? col.stage.default_probability) != null ? `${Math.round(card.win_probability ?? col.stage.default_probability)}%` : '—'}</td>
                    <td className="num">{card.amount ? Math.round(card.amount).toLocaleString('en-IN') : '—'}</td>
                  </tr>
                )))}
                {totalDeals === 0 && <tr><td colSpan={5} className="tiny">No deals yet.</td></tr>}
              </tbody>
            </table>
          </div>
        )}
              ...
              <tbody>
                {columns.map((col) => (
                  <tr key={col.stage.id}>
                    ...
                    <td>{col.cards.length}</td>
                    <td>{Math.round(col.stage.default_probability)}%</td>
                    <td className="num">{Math.round(col.total_value).toLocaleString('en-IN')}</td>
                    <td className="num">{Math.round(col.total_value * (col.stage.default_probability / 100)).toLocaleString('en-IN')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div className="ai-frame" style={{ padding: 13, margin: 13 }}>
